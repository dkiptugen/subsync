...
//                                                      ->orWhere('enddate', 'LIKE', "%{$search}%")
            ...
            if ($validateddata) {
                if ($type == 'organization') {
                    if (! $request->has('organization')) {
                        return self::failed('Whitelist', 'Organization not selected', route('whitelist.type.index', $type));
                    }
                    ...
                    try {
                        ...
                        return self::failed('Whitelist', $e->validator->errors()->first(), route('whitelist.type.index', $type));
                    }
                    ...
                    //                                            {
                    //                                                return self::failed('Whitelist', 'Email not set', route('whitelist.type.index', $type));
                    //                                            }
                    ...
                    if ($count > 0) {
                        ...
                        return self::failed('Whitelist', 'No email addresses supplied', route('whitelist.type.index', $type));
                    }
                    ...
                    if ($users->isEmpty()) {
                        return self::failed('Whitelist', 'User/ users not found in the database, ask the user/users to register first', route('whitelist.type.index', $type));
                    } else {
                        if ($count > 10) {
                ...
                return self::failed('WhiteList', 'non existent route', route('whitelist.type.index', $type));
            } else {
                return self::failed('Whitelist', $validateddata, route('whitelist.type.index', $type));
            }

        } catch (Exception $e) {
            return self::failed('Whitelist', $e->getMessage(), route('whitelist.type.index', $type));
        }
     ...
     *
     * @param  UserWhitelist  $userWhitlist
     * @return Application|Factory|\Illuminate\Contracts\View\View|View
     */
