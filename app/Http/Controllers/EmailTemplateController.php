							...
							if ($validatedata)
								{
									...
									return self::fail ('Email Template', 'Unable to save',
							...
							else
								{
									...
									return self::fail ('Email Template', 'Error encountered contact the developer',
									                   route ('email_template.index'));
								}
						}
					catch (\Exception $e)
			...
			public function edit (EmailTemplate $email_template)
				{
					$this->data['products'] = Product::get ();
					$this->data['types'] = EmailType::cases ();
					$this->data['variables'] = config ('email.'.EmailType::from
