        ...
        public function store(StoreSubscription $request)
            {
                ...
                if (is_null($user))
                    {
                        return self::fail('Subscription', 'This user does not have an account, create his/her account', route('product.subscriber.create', 0));
                    }
                if (is_null($rate))
                    {
                        return self::fail('Subscription', 'This subscription type is not available for this product', route('subscription.index'));
                    }
                else
                    {
